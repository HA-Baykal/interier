...
**Четыре пункта `deepseek/…` с подписью *From gateway*.** Моделей на самом деле две,
FCC показывает каждую в двух режимах:

| Пункт в списке | Что это |
| --- | --- |
| `deepseek/deepseek-flash` | DeepSeek-V4.1-Flash, режим thinking (модель сначала рассуждает, потом отвечает). Рабочий вариант по умолчанию. |
| `deepseek/deepseek-flash (no thinking)` | та же модель, FCC отправляет `thinking: disabled` — быстрее и дешевле, для мелких правок и простых вопросов. |
| `deepseek/deepseek-v4-pro` | DeepSeek-V4-Pro (1.6T MoE), thinking. Сильнее в знаниях/сложных рассуждениях, но ~4× дороже вход и ~3× выход; по коду и агентным задачам V4.1-Flash её уже догнал. Не понимает картинки. |
| `deepseek/deepseek-v4-pro (no thinking)` | Pro без рассуждений. Практически незачем: Flash в thinking-режиме дешевле и не хуже. |

Технически «(no thinking)» — это id вида `claude-3-freecc-no-thinking/deepseek/…`:
Claude Code считает модели `claude-3-*` не умеющими думать и не показывает рассуждения,
а FCC выключает thinking на стороне DeepSeek. Обычный пункт (`anthropic/deepseek/…`)
передаёт уровень усилий из Claude Code: `max`/`xhigh` → `reasoning_effort: max`,
остальные → `high` (у DeepSeek `high` — значение по умолчанию). Токены рассуждений
оплачиваются как выходные. Выбор пункта *From gateway* действует только для текущей
сессии чата; `MODEL` в Admin UI при этом не меняется.

**Шкала рядом с полем ввода и `/compact`.** Шкала показывает заполнение **контекста** —
рабочей памяти модели в текущем чате (все сообщения, прочитанные файлы, вывод команд).
Это не деньги и не лимит запросов: когда шкала заполнится, Claude Code сам сожмёт историю
в резюме и продолжит (то же делает команда `/compact` вручную). FCC выставляет порог
авто-сжатия `CLAUDE_CODE_AUTO_COMPACT_WINDOW=190000` — шкала рассчитана примерно на 200k,
хотя сама DeepSeek принимает 1M: длинный контекст дороже и ухудшает точность. Что влияет
на баланс DeepSeek: каждое сообщение отправляет модели всю историю заново (повторяющаяся
часть идёт по цене кэша, ~в 50 раз дешевле), плюс ответ и рассуждения. Практика: `/compact`
после завершённой задачи, `/clear` перед несвязанной, правила и договорённости — в
`CLAUDE.md` в папке проекта, чтобы они переживали сжатие. Подпись *Extra high* у модели —
уровень усилий рассуждений (у DeepSeek это `reasoning_effort: max`): для рутинных правок
достаточно *High*, переключается в той же плашке модели / `/model`.
...
| Доступ вернулся (сайт DeepSeek открывается без VPN), но Claude Code в VS Code по-прежнему не отвечает; перезапуск VS Code не помог | Перезапускать нужно не VS Code, а прокси: иконка FCC в трее → **Restart Server** (или **Quit** и запустить заново). Проверка: в браузере `http://localhost:8082/health` → `{"status":"healthy"}`. Затем Admin UI → Providers → DeepSeek → **Refresh models**: число моделей = FCC достучался до API; «Could not load models» = API ещё недоступен (сайт `chat.deepseek.com` и API `api.deepseek.com` — разные хосты, проверка в PowerShell: `curl.exe -m 15 -s -o NUL -w "%{http_code}" https://api.deepseek.com/models` → `401` = доступен, `000` = нет; при `000` — `ipconfig /flushdns`, снять `$env:HTTPS_PROXY`, иначе снова VPN). Дальше отделите VS Code от FCC: в терминале `fcc-claude` → «привет». Отвечает → проблема в интеграции: Admin UI → Integrations → Claude Code (VS Code) → Connect/Refresh, в VS Code `Ctrl+Shift+P` → Developer: Reload Window, начать **новый** чат. Точная причина — в `%USERPROFILE%\.fcc\logs\server.log` (последние строки). |

...
import { uploadStorageMode, isBlobStorage } from "@/app/api/upload/service";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const probe = !probeRequested ? null : cfg.apiKey
      ? await probeCompatible(cfg)
      : { ok: false, keyAccepted: false, message: "API-ключ не задан." };
    return NextResponse.json({
      ...
      storage: { database: storageMode(), uploads: uploadStorageMode(), blobConfigured: isBlobStorage() },

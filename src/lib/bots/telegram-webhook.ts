...
import { handleTelegramPaymentUpdate } from "../payments-stars";
...
export async function dispatchTelegramWebhook(req: Request, update: unknown): Promise<void> {
  try {
    ...
    // The shared helper also serves the long-polling fallback.
    if (await handleTelegramPaymentUpdate(update)) return;

...
/**
 * Returns `true` when the update belonged to the login flow. Everything else
 * must go on to the messenger bot engine (`dispatchTelegramUpdate`), which is
 * what makes the same bot a full application and not only a login prompt.
 */
export async function handleTelegramAuthUpdate(update: unknown): Promise<boolean> {
  if (!update || typeof update !== "object") return false;
  let cfg: TelegramConfig;
  try {
    cfg = telegramConfig();
  } catch {
    // No environment token (the bot token may live only in the admin panel):
    // the login transport is off, but the bot application still answers.
    return false;
  }
  ...
  if (message?.chat?.type === "private" && typeof message.text === "string") {
    ...
    if (!person || String(message.chat.id) !== person.id) return false;
    const match = message.text.match(/^\/start(?:@[A-Za-z0-9_]+)?\s+auth_([a-f0-9]{32})$/);
    // Anything that is not an `auth_` ticket is the bot app's business: menu,
    // styles, photo, "change only the curtains", shopping links, history.
    if (!match) return false;
    try {
      const state = await updateFromBot(match[1], cfg, person, "start");
      if (state.status !== "waiting") { await telegramCall(cfg, "sendMessage", { chat_id: person.id, text: "Этот запрос уже обработан. Вернитесь на сайт или начните новый вход." }); return true; }
      ...
      if (e instanceof RequestError && e.status < 500) { await telegramCall(cfg, "sendMessage", { chat_id: person.id, text: e.message }); return true; }
      throw e;
    }
    return true;
  }
  const callback = body.callback_query;
  if (!callback || typeof callback.id !== "string" || typeof callback.data !== "string") return false;
  const match = callback.data.match(/^auth:(approve|deny):([a-f0-9]{32})$/);
  const person = personFrom(callback.from);
  if (!match || !person || callback.message?.chat?.type !== "private" || String(callback.message.chat.id) !== person.id || String(callback.message.from?.id) !== cfg.botId) return false;
  try {
    ...
    if (e instanceof RequestError && e.status < 500) { await telegramCall(cfg, "answerCallbackQuery", { callback_query_id: callback.id, text: e.message.slice(0, 180), show_alert: true }); return true; }
    throw e;
  }
  return true;
}
